...
function handleSubmit(e: Event) {
	e.preventDefault();
	console.log('handleSubmit called - email:', email, 'password len:', password.length);
	if (!email || !password) {
	...
	isSubmitting = true;
	userStore.signIn(email, password).finally(() => {
		isSubmitting = false;
	});
}
</script>
  ...
  <Card.Root class="w-full max-w-md md:max-w-2xl lg:max-w-3xl overflow-hidden p-0">
