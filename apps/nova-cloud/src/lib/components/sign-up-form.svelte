...
function handleSubmit(e: Event) {
	...
	isSubmitting = true;
	userStore.signUp(email, password).finally(() => {
		isSubmitting = false;
	});
}
</script>

  <div class={cn("w-full max-w-md md:max-w-2xl lg:max-w-4xl mx-auto flex flex-col gap-6", className)} {...restProps}>
  <Card.Root class="overflow-hidden p-0">
      ...
      <form class="p-6 md:p-8" onsubmit={handleSubmit}>
        <FieldGroup>
          ...
          <Field>
            ...
            <Input id="confirm-password-{id}" type="password" bind:value={confirmPassword} placeholder="Confirm your password" required />
          </Field>
          <Field>
            <Button type="submit" disabled={isSubmitting || userStore.isLoading}>
	{#if isSubmitting || userStore.isLoading}
	...
	{/if}
          ...
          <FieldDescription class="text-center">
            Already have an account? <a href="/auth/sign-in" class="underline-offset-2 hover:underline">Sign in</a>
          </FieldDescription>
        </FieldGroup>
      </form>
