			...
			{#if scheduledRuns.length > 0}
				<div class="space-y-3">
					{#each scheduledRuns as run (run._id)}
						<a href={`/studios/${data.studio._id}/chat/${run.chatId}`} class="flex items-center justify-between gap-4 rounded-[1.25rem] border border-border/60 bg-background/75 px-4 py-4 transition-colors hover:bg-muted/40">
							<div class="min-w-0">
								<div class="flex flex-wrap items-center gap-2">
