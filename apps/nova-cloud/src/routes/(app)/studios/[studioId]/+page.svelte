		...
		<section class="rounded-[2.25rem] border border-border/70 bg-background/85 p-6 shadow-sm backdrop-blur sm:p-8">
			<div class="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
				...
				<div class="flex flex-wrap gap-3">
					<Button variant="outline" class="rounded-full px-5" href={`/studios/${data.studio._id}/sandbox`}>
						Open sandbox
					</Button>
		...
		<div class="grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
			<section class="rounded-[2rem] border border-border/70 bg-background/85 p-6 shadow-sm backdrop-blur">
				<div class="mb-5 flex items-center justify-between gap-4">
					...
					<Button variant="ghost" class="rounded-full" onclick={() => goto(`/chats?studio=${data.studio._id}`)}>
						See all
					</Button>
				</div>
			...
			<div class="space-y-5">
				<section class="rounded-[2rem] border border-border/70 bg-background/85 p-6 shadow-sm backdrop-blur">
					...
					<div class="mt-4 flex flex-wrap gap-3">
						<Button variant="outline" size="sm" class="rounded-full" href={`/studios/${data.studio._id}/sandbox`}>
							Open sandbox
						</Button>
				...
				<section class="rounded-[2rem] border border-border/70 bg-background/85 p-6 shadow-sm backdrop-blur">
					<div class="mb-5 flex items-center gap-3">
						...
						<div class="flex gap-2">
							<Button variant="outline" size="sm" class="rounded-full" href={`/studios/${data.studio._id}/deployments`}>
								Manage
							</Button>
								...
								<div class="rounded-[1.25rem] border border-border/60 bg-background/75 p-4">
									<div class="flex items-start justify-between gap-4">
										...
										<a
											...
											href={`/studios/${data.studio._id}/chat/${run.chatId}`}
										>
											Open chat
										</a>
									</div>
					...
					{#if enabledIntegrations.length > 0}
						<div class="space-y-3">
							{#each enabledIntegrations as integration (integration._id)}
								<a href={`/studios/${data.studio._id}/integrations/${integration.key}`} class="flex items-center justify-between rounded-[1.25rem] border border-border/60 bg-background/75 px-4 py-3 transition-colors hover:bg-muted/40">
