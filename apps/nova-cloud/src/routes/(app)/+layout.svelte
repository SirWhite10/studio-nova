      ...
      return [
        {
          ...
          href: pathSegments.length > 1 ? '/chats' : undefined,
        },
      ];
    }

    if (pathSegments[0] === 'studios') {
      const studioLabel = sidebarData?.currentStudio?.name ?? 'Studio';
      const studioHref = studioId ? `/studios/${studioId}` : undefined;
      const remaining = pathSegments.slice(2);
