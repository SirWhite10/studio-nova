<script lang="ts">
	import * as Chart from "$lib/components/view-ui/chart/index.js";
	import * as Card from "$lib/components/view-ui/card/index.js";
	import * as Select from "$lib/components/view-ui/select/index.js";
	import * as ToggleGroup from "$lib/components/view-ui/toggle-group/index.js";
	...
	const chartConfig = {
		desktop: { label: "Desktop", color: canvasTheme.colors.primary },
		mobile: { label: "Mobile", color: canvasTheme.colors.primary },
	} satisfies Chart.ChartConfig;

	let timeRange = $state("90d");
	const mobileQuery = new IsMobile(768);
	const isMobile = $derived(mobileQuery.current);
	let viewportWidth = $state(1280);

	const horizontalPadding = $derived(viewportWidth >= 1024 ? 6 : 4);
	const chartContentPadding = $derived.by(() => {
		const horizontal = viewportWidth >= 640 ? "1.5rem" : "0.5rem";
		const top = viewportWidth >= 640 ? "1.5rem" : "1rem";
		return `padding-left: ${horizontal}; padding-right: ${horizontal}; padding-top: ${top};`;
	});
...
<View paddingLeft={horizontalPadding} paddingRight={horizontalPadding}>
	<Card.Root>
		<Card.Header>
			...
			<Card.Action>
				{#if isMobile}
					<Select.Root type="single" bind:value={timeRange}>
						<Select.Trigger size="sm" style="width: 10rem;" aria-label="Select a value">
							<span data-slot="select-value">{selectedLabel}</span>
						</Select.Trigger>
						<Select.Content>
							<Select.Item value="90d">Last 3 months</Select.Item>
							<Select.Item value="30d">Last 30 days</Select.Item>
							<Select.Item value="7d">Last 7 days</Select.Item>
						</Select.Content>
					</Select.Root>
				{:else}
					<ToggleGroup.Root type="single" bind:value={timeRange} variant="outline" spacing={0}>
		...
		<Card.Content style={chartContentPadding}>
			<Chart.Container config={chartConfig} style="width: 100%; height: 250px;">
				<AreaChart
					legend
					data={filteredData}
					x="date"
					...
					{#snippet marks({ context })}
						<defs>
							<linearGradient id="fillDesktop" x1="0" y1="0" x2="0" y2="1">
								<stop offset="5%" stop-color="var(--color-desktop)" stop-opacity={1} />
						...
						{#each context.series.visibleSeries as series (series.key)}
							<Area
								...
								motion="tween"
								{...series.props}
								fill={series.key === "desktop" ? "url(#fillDesktop)" : "url(#fillMobile)"}
							/>
						{/each}
					{/snippet}
					{#snippet tooltip()}
